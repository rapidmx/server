...
import { AppearancePreferencesMongo, BrandingMongo } from "@rapidmx/restapi/mongo";
...
export class WwwRoute extends ReactRoute {
    ...
    protected async fetchProps(req: HttpRequest): Promise<any> {
        ...
        const appearance = await fetchAppearanceForSSR(this.brandingObjectFactory, AppearancePreferencesMongo, req.user?.uid, this.appearanceLogger);
        return {
