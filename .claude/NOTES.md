...
### 2026-09-21 - wiring for appearance preferences and background send (the implementation and contracts are in `@rapidmx/restapi`'s NOTES, 2026-09-21)

Not committed. Server side of the web-client UX batch; nothing here is logic, it is mounting and configuration:
- **Routes:** `src/{mongo,sql}/routes/AppearanceRoute.ts`, one-line subclasses of restapi's `AppearanceRouteMongo/SQL` at `@ApiRoute("mail/preferences/appearance")` -> `/api/mail/preferences/appearance` (`GET`, `PUT`, `POST/DELETE /background`,
  `GET /background/:version`). `MessageRoute` needed nothing: restapi's `MessageRouteMongo/SQL` now carry `sendJobClass`, and the server already mounts `ScheduledSendJob{Mongo,SQL}` (`src/{mongo,sql}/Jobs.ts`) - the route finds the manager's instance with
  `objectFactory.getInstance(ScheduledSendJobMongo)`, so the same instance (and pool) serves the scheduled tick and the background kicks. **Everything runs in one process here** (`server.ts` only supervises `worker.ts`), which is what the in-process kick relies on.
- **Models:** `AppearancePreferencesMongo/SQL` added to `src/{mongo,sql}/Models.ts` (`test/Models.test.ts` fails until they are). New collection/table + unique index on the next start (`synchronize`), no migration.
- **Page prop:** both `wwwRoute.ts` files return `appearance` = `fetchAppearanceForSSR(objectFactory, AppearancePreferences{Mongo,SQL}, req.user?.uid, logger)` - `undefined` for no row / not signed in, never throws (debug log). **Known limit:** `ReactRoute`
  caches the rendered page in Redis for 60 s per user in production (`hashRequest()` = path + query + `userUid`), so after a change the prop can be up to a minute stale on a reload; the web client has to apply the live event / its own copy on top.
- **Config** (`config.mongo.ts`/`config.sql.ts`, both): `mail.preferences.background_max_bytes` = `DEFAULT_APPEARANCE_BACKGROUND_MAX_BYTES` (8 MiB, `config.defaults.ts`; the server's `max_body_size` is 100 MiB so the route's 413 is the effective limit) and
  `mail.jobs.scheduled_send.{concurrency: 4, drain_ms: 15000}` merged into the **existing** `mail.jobs` block (a second `jobs:` key in the object literal silently replaces the first - my first attempt did exactly that; `test/routes/AppearanceRoute.test.ts` reads the values through the
  shipped configs so it can't come back). `mail:jobs:scheduled_send:*` is otherwise defaulted by restapi's `@Config` decorators and appears nowhere else in this repo.
- **Verifying against the working tree:** this repo installs `@rapidmx/restapi` 0.16.0 as a *copy* (`node_modules/@rapidmx/restapi`), so `yarn tsc`/`yarn test` here need restapi's built `dist` overlaid on it (`yarn tsc` in restapi, then copy `dist/` over) until a release is
  published and the dependency bumped - the overlay is in place from this session (original 0.16.0 dist not kept; `yarn install --force` restores it).
- Tests: `test/routes/AppearanceRoute.test.ts` (both routes' inherited handlers and paths, `sendJobClass`, the config values, and `fetchProps` returning / omitting / surviving a failing read of `appearance`); `Models.test.ts` covers the model registration.

### 2026-09-21 - PRIVACY: an administrator no longer reads other users' mail (server side of restapi's "no role reads another user's mail")

The implementation, the policy, the decisions and the full route audit are in `@rapidmx/restapi`'s NOTES (2026-09-21, "PRIVACY: no role reads another user's mail"); this is what changed or was checked here.

- **Cause seen on the live host:** an administrator holding an elevated token (`roles: ['admin']`) got every mailbox's folders and messages through `/api/mail/*`, and the web client's mailbox switcher / Settings "Mailbox" dropdown listed everybody. A sign-in that did not elevate never had it. Nothing about mounting or config caused it - `trusted_roles` is unchanged - it was `ACLUtils.hasPermission()` treating a trusted role as a superuser.
- **`ACLRoute` (`/api/acls`)** now extends `src/routes/BaseGuardedACLRoute.ts` (Mongo and SQL) instead of service-core's `BaseACLRoute`. `BaseACLRoute` let any trusted caller read and write every ACL document, so `PUT /api/acls/<mailbox>` added the administrator to any mailbox and `PUT /api/acls/Mailbox` (the class ACL every mailbox ACL inherits from) could have opened all of them. Decision: an ACL that governs mail - a mailbox's, a folder's (its parent chain reaches the `Mailbox` class ACL) or the `Mailbox` class ACL itself - is readable/writable only by a caller who holds `FULL` on it AS THEMSELVES (`hasMailAccess()`: the owner, or a `manager` delegate; never through a trusted role), and is left out of `GET /api/acls` for everyone else; the class ACL is changeable by no caller. Every other ACL (other models' class ACLs, route ACLs) is exactly as before. The audited way an administrator shares a mailbox is restapi's `/mail/mailboxes/:id/access` (list any mailbox's members, revoke any, grant on an ownerless one), which the admin console now uses. Not inspected: the body of a `POST /api/acls` (`BaseACLRoute.checkPerms` sees route parameters only) - the framework refuses a create at a uid that already has an ACL, so it grants nothing over existing mail.
- **`BaseMessageRawContentRoute` (`GET /:id/raw`) and `BaseMailComposeRoute` (`assemble`, `assembleRaw`)** used `aclUtils.hasPermission(user, folderUid, ...)` directly; they now call `hasMailAccess()` (`@rapidmx/restapi`), so a trusted role gets 404 / 403 there. `BaseEscrowInfoRoute` already had no trusted bypass (owner or explicit ACL record); a mailbox that doesn't exist is now 403 like one the caller can't access.
- **`PushRoute`** is `MailPushRoute` (restapi): channel subscriptions and publishes no longer go through the trusted-role shortcut. **`wwwRoute`/`AdminConsoleRoute`** only pass `trusted`/`trustedRoles` to the page shells; they serve no mailbox data. **`DevImpersonationRoute`** (dev only) mints an unprivileged token for the target - the same shape as auth-server's real `BaseImpersonationRoute` (target's own roles, not elevated) - so impersonation keeps working: an impersonation token is simply the target's identity.
- **Audit of this repo's routes** (`test/routes/MailAccessRoutes.test.ts` fails on an unclassified route or a direct `aclUtils.hasPermission()`): every class in `src/{mongo,sql}/routes` is one of restapi's (classified in restapi's `mailAccessRouteTable.ts`, put through its access matrix) or the server's own - mailbox-scoped: `MessageRawContentRoute`, `MailComposeRoute`, `EscrowInfoRoute`; admin / no data: `ACLRoute`, `AdminConsoleRoute`, `AdminRoute`, `EscrowConsoleRoute`, `MetricsRoute`; user: `GiphySearchRoute`, `wwwRoute`; public: `OpenAPIRoute`, `PublicPageRoute`, `StaticAssetRoute`, `StatusRoute`. Plugins (read-only here): `activesync` (~25 direct `aclUtils.hasPermission(ctx.user, folderUid, ...)` calls on client-supplied folder ids) and `booking-plugin` (`BaseBookingProfileRoute`, `BaseBookingTypeRoute`) still use the framework's shortcut - a caller whose token carries a trusted role could reach other users' folders through them; both resolve the caller's own mailbox by `ownerUserUid` (as `mapi` and `autodiscover` do) and a device signs in with its user's normal token, so it is latent; they should move to `hasMailAccess()` in their next release.
- **Rollout:** after deploying, administrators do not need to do anything: an elevated session simply stops showing other people's mail. Users' mail was never exposed to non-elevated sign-ins. To see an account an administrator uses "Impersonate this user" on the mailbox page (the impersonation banner is unchanged); a shared mailbox such as `hello@` appears in the administrator's mail client after they add themselves under Shared access. No migration, no config change.
- **Verifying against the working tree:** restapi's built `dist` is overlaid on `node_modules/@rapidmx/restapi` (see the appearance entry above); rebuilt from the working tree after this change.
- Tests: `test/routes/BaseGuardedACLRoute.test.ts` (mail ACLs refused to a trusted caller, allowed to the owner, others untouched, bulk and list), `BaseMessageRawContentRoute.test.ts` / `BaseMailComposeRoute.test.ts` (the ACL check is handed the caller without trusted roles; an administrator gets 404 / 403), `MailAccessRoutes.test.ts` (route classification and the no-direct-`hasPermission` scan), `BaseEscrowInfoRoute.test.ts` (missing mailbox -> permission failure).

### 2026-09-21 (later) - a shared-mailbox grant typed as a username matched nobody: resolved on the server, validated on `/api/acls`

Cause and design are in restapi's NOTES ("a shared mailbox grant stored against a username matched nobody"). Here: `BaseGuardedACLRoute` (`/api/acls`) now also refuses (400 `No user found for "<x>".`) any create/update/updateProperty/updateBulk of a mail ACL whose records name a principal that is not a lowercase user uid, unless that exact id is already stored on the
ACL (a full object can still be saved back) - tests in `test/routes/BaseGuardedACLRoute.test.ts`. Sharing itself (`PUT /api/mail/mailboxes/:id/access/:principal`) resolves an address, an auth-server username/e-mail alias or a uid against `mail:auth_server_url` (already configured for self-service mailbox creation) with the caller's cookie; nothing to configure. **The live `hello@powerlevel.gg` ACL still holds the bad record
`jean-philippe`** (no migration): as an administrator open the mailbox page in the console, use "Replace with a user" on that entry (it looks the name up, shows the person, grants the uid, removes the string), or "Add me" if you just want it in your own mail client.

### 2026-09-21 (P1) - uninstalling a plugin can delete its data (`purgeData`), safely, once no server runs it

JP: "when uninstalling a plugin, there should be a checkbox option to delete all plugin data". Nothing committed, no version bumps. Web-client/react-shared halves are in their own NOTES; booking got a `./purge` hook.

- **Why it lives in the server, not restapi.** The uninstall is restapi's `BasePluginRoute.remove()` (`DELETE /system/plugins/:id`, row `removed: true`, audit `plugin.remove`, announce) and the `Plugin` model has no room for purge state. The server's `PluginRoute` (mongo + sql) is now `withPluginPurge(PluginRouteMongo|SQL, { datastore, purgeClass })` (`src/routes/PluginPurgeRoutes.ts`), a mixin that overrides `remove`/`add`/`status` (same decorators and parameter positions as restapi's; the body is read from `req.body`, the framework doesn't re-index inherited parameter metadata) and adds `POST /purges/:uid/retry`. The mixin is generic over the base, so `test/routes/PluginPurgeRoutes.test.ts` runs it over a fake base with a real SQLite ledger; `test/routes/PluginRoute.mongo.test.ts` runs the real thing over HTTP on a real Mongo (JWTs, real audit rows). Worth moving into restapi next release (new `AuditAction` values `plugin.purge_*` - they're plain strings here - and `purgeData` on `remove()`).
- **State: `PluginPurgeMongo`/`PluginPurgeSQL`** (`src/{mongo,sql}/PluginPurge*.ts`, one row per plugin name, **not** in `Models.ts` - `Models.test.ts` pins that file to restapi's models). The row holds the inventory (what the plugin's models are stored in), the request (who/when/`wasEnabled`/version/integrity), the lease (`leaseOwner` = a unique claim token, `leaseExpiresAt`), `steps` and `error`. SQL needed `@Column({ type: "varchar" })` on `state` (a string-union design type is `Object`, which better-sqlite3 rejects - same note as restapi's models). `PluginPurgeStore` = compare-and-set `update(name, { state?, leaseOwner? }, patch)` (Mongo `updateOne` `matchedCount`, SQL `repo.update` `affected`; equality conditions only, no date comparisons in the database); `PluginPurgeLedger` builds the transitions on it (request/cancelPending/claim/renew/saveSteps/finish/release/retry/clearOutcome), tested on real Mongo and real SQLite including two/three copies racing a claim, a lapsed lease taken over by exactly one, and a cancel racing a claim.
- **What is deleted (`PluginPurger`, started by `PluginHost.start()`).** After the plugin's `onPurge` hook: every collection/table of the plugin's model classes (emptied, then dropped; Mongo via `connection.db`, SQL via the query runner), the plugin's saved settings (the removed row stays - it is what stops `system:plugins:defaults` re-adding it), this copy's package dir and cached `.ui-build/<hash>` folders whose manifest names the plugin. Steps `{step, ok, count?, error?, note?}` are stored after each one; a failure doesn't stop the others; `settings` and `files` only run when every earlier step succeeded; a retry (`failed -> pending`) re-runs only failed steps. **Owned collections are derived when a copy loads the plugin** (`PluginClassLoader.classesOf()` -> `describeModels()`, names from the live connection: Mongo `collectionNameFor`, SQL `getMetadata().tablePath`) and recorded (grows only, so an older version's collections stay known) - after an uninstall nothing is loaded, and the plugin may have been disabled for ages. A plugin that never loaded since this release therefore has no inventory: `purgeData` is refused with a 409 at request time (nothing changed) rather than silently deleting nothing. **Guard** (`PluginOwnedData.assertPurgeable`): refuses the `acl` datastore, non-identifier names, `system.*`, anything a core class (the class loader's non-plugin classes + service-core's ACL models) or another plugin's inventory resolves to, case-insensitively and by bare name for schema-qualified tables - a plugin class extending `FolderMongo` resolves to `folder_mongo` and is refused (tested).
- **Never while a server runs it (`PluginPurger.gate`).** Removed row; not loaded on this copy; `wasEnabled` -> wait `system:plugins:purge:grace_ms` (3 min, covers a copy invisible between reading the plugin table and its first heartbeat); if a cache is configured, read `plugins:status` (fail closed when unreadable): every fresh report (< `PLUGIN_STATUS_MAX_AGE_MS`) must not list the plugin as loaded, not carry an error naming it, and - unless it's in safe mode - carry the current desired hash (i.e. it restarted into the set without the plugin: this is each server's "ack"; `N of M servers` in the UI is the same computation, `blockingInstances()`); the restart lock must be free; no `plugins:starting` entry younger than 15 min (new: `PluginHost.resumeRestartLock` marks the copy starting, `start()` clears it). No cache -> single copy, only "not loaded here". The gate is re-run before every step; a copy that turns out to run the plugin again releases the lease (`pending`, steps kept). Lease `system:plugins:purge:lease_ms` (2 min), renewed every third of it and per step; `stop()` finishes the current step and releases.
- **Reinstall.** `POST /` (the mixin) cancels a pending purge of the plugin - and, when any purge is unfinished, of every plugin `GET /plan` says it will revive - before calling restapi's `add`, restores them if `add` fails, refuses with 409 while one is *running* (even with a lapsed lease: another copy resumes it; waiting is safer than racing), and answers `purgeCancelled` + `warnings`. Uninstalling again without `purgeData` clears an old done/failed/cancelled outcome so a stale "Data deleted" row can't describe new data.
- **Hook.** `package.json` `exports["./purge"]` -> `onPurge(ctx)` (`PluginPurgeHook.ts`). The plugin isn't loaded anywhere, so `InstallingPurgeHookRunner` installs the recorded version+integrity into `<plugins dir>/.purge/<name>` with a normal `PluginInstaller`, imports the module, runs it with a timeout and an `AbortSignal` (`system:plugins:purge:hook_timeout_ms`, 60 s), deletes the scratch dir. A hook failure is recorded and the rest goes on; an error named `AbortPurgeError` (or `abortPurge: true`) stops everything (plugins can't import the server's class). Booking's hook deletes its profile images from the BlobStore by the keys in its profile rows and aborts if it can't - the rows are the only record of the keys.
- **Files (`PluginPurgeFiles.removeContained`).** Only inside the plugin directory's `node_modules` / `.ui-build`; refuses the root, a target that is or contains a link/junction, a link anywhere between root and target, or whose realpath leaves the root. Tests create junctions and remove only the link (`fs.rmdirSync`) before cleaning up.
- **Audit.** `plugin.purge_requested` (actor, plugin uid, `purgeData`, version, `wasEnabled`) beside restapi's `plugin.remove`; `plugin.purge_cancelled` (on the revived row's uid), `plugin.purge_retried`, and from the purger `plugin.purge_completed`/`plugin.purge_failed` (no actor; `requestedBy` in details, per-step summary). Real rows verified over HTTP on Mongo.
- **API.** `DELETE /:id` `{purgeData: true}` (or `?purgeData=true`; `"yes"`/`1`/`null` -> 400) needs `req.user.elevated > 0` (403 `api-104` like `@RequiresElevation`, but only when the flag is set; the plain call needs the trusted role only, as before) and answers `{ purgeScheduled, purge? }` (older clients ignore it). `GET /status` gained `purges` (uninstalled plugins only, `pending|running|done|failed`, with `serversRunning/serversTotal` while pending). `POST /purges/:uid/retry` (elevated, only `failed`).
- **What each bundled plugin owns.** booking: `booking_type_*`, `booking_*`, `booking_profile_*` + profile images `booking-profiles/{avatar,banner}/<uuid>` (hook); its bookings' `CalendarEvent`s are the owners' own and stay. activesync: `device_sync_state_*`, `eas_collection_state_*`, `eas_collection_chunk_*`; mail it sent lives in mailboxes (its `blobStore.put` keys are core message bodies - **never** delete blobs by a `*BlobKey` column generically); its Redis leases expire. mapi: no models (`MapiSessionManager` keeps short-lived Redis handle data). autodiscover: nothing. BlobStore has no listing and no plugin prefix convention (booking's `booking-profiles/` is by habit) - the server deletes no blobs itself.
- **Not done / to flag.** Purge state and inventory are per deployment database; the package and `.ui-build` cleanup only reaches the copy that runs the purge (others prune at their next start - npm prunes the package at the restart that applied the removal; a UI-less start keeps old builds). Booking, restapi and `AuditAction` need releases for a published deployment to have the hook / enum. `system:plugins:purge:*` keys aren't in the Helm chart. Notifications in the admin UI need W-C's `notify` (present in the tree, untracked).

Verified: `yarn tsc --noEmit`, `yarn lint`, `yarn test` (44 files, 572 tests at the time), and the new suites: `PluginPurgeStore` (26, real Mongo + SQLite), `PluginOwnedData` (14), `PluginPurger` (29), `PluginPurgeFiles` + `PluginPurgeHook` (22), `PluginPurgeCoordination` (8), `PluginHostPurge` (2, real SQLite + a real plugin module), `PluginPurgeRoutes` (28), `PluginRoute.mongo` (9, over HTTP on a real Server + Mongo).

**Real run (scratch stack, not docker).** Docker Desktop's daemon was unresponsive (`docker ps` answered, `pull`/`images` hung), so the run used the repo's own memory servers: a real mongod (mongodb-memory-server) and Redis (Memurai, redis-memory-server), the real server from source (`node --import tsx src/server.mongo.ts`, supervisor + worker), the real `@rapidmx/booking-plugin` from an `npm pack` tarball (`system__plugins__sources`; UI built by Vite in 2 s) and real HTTP - 16/16 checks. The plugin installs and loads and its inventory (three collections + hook) is recorded; booking type/booking/profile rows, a profile image blob and a core `folder_mongo` row are inserted; `DELETE ... {purgeData:true}` answers `purgeScheduled` with `serversRunning: 1` and audits `plugin.remove` + `plugin.purge_requested`; the worker restarts itself and the purge runs on the restarted copy in one attempt (hook ok, the three collections emptied and dropped with counts 1/1/1, blobs note, settings, files 1); afterwards `booking_*` are gone, `folder_mongo` is intact, the image file was deleted by the hook (installed from the tarball into `.purge/`), the row is kept `removed`, `plugin.purge_completed` is audited, the package dir was pruned, and a further start keeps it `done` with booking not loaded. Gotchas: the datastore's `database` config (`rrst_auth`) wins over the database in a `datastores__mongo__url` path; under tsx the dev auto-login answers every request as an elevated admin (so 401/403/elevation are covered by the HTTP tests, not by this run); `system:plugins:dir` must sit under the server folder or every plugin is refused for resolving its own `@rapidrest/core` (the run used `server/.p1-qa-plugins`, removed afterwards). Not exercised for real: SQL (covered on SQLite in tests), several server copies at once (covered by the ledger race tests and the gate tests with a faked status), a browser (no harness).

## 2026-09-21 — Config for the RFC 8823 enrollment status (R3)

Wiring only for `@rapidmx/restapi`'s folder and signing-enrollment changes (see its `.claude/NOTES.md`, 2026-09-21): `config.mongo.ts`/`config.sql.ts` gain `mail.pki.rfc8823.poll_interval_seconds` (300 - what the
enrollment status's `nextCheckAt` is computed from; keep it equal to `jobs.acme_enrollment_driver.schedule`) and `max_pending_hours` (168 - a request the CA never answered is failed as `order-expired` after this
long when the CA gave no expiry). No route, model or job changed here: the routes are re-exported from restapi (`FolderRoute`, `KeyVaultRoute`, `MailboxRoute`), so `GET /api/mail/mailboxes/:id/keyvault/keys/sign-enrollment`
(current enrollment) and `POST .../sign-enrollment/:enrollmentId/check` appear with the package. Verified against restapi's built `dist` copied over `node_modules/@rapidmx/restapi/dist`.

## 2026-09-21 (later) — Signing certificates: chart backend selection, the admin route, honest status (R6)

JP's live report: requested a signature key over an hour ago, no key, no status or failure info anywhere. **Diagnosis (verified live):** the deployment ran the default
`mail.pki.signing_enrollment.backend: "manual"` (`config.{mongo,sql}.ts`), whose enrollment sat `pending` in `/var/lib/rapidmx/pki/manual-enrollments.json` waiting for a human to upload a certificate -
but restapi shipped no admin route for that upload step - while Settings > Encryption told the user a public CA issues it automatically in the background (true only for `rfc8823`, never turned on).
JP decided: switch the live deployment to automatic issuance and build the manual-completion path too. See restapi's `.claude/NOTES.md` (2026-09-21, same title) for the code-level contracts; this
entry covers what changed here (chart, config, route mounts, docs). Not verified here either: a real CA e-mail exchange (only `helm lint`/`helm template` and the built restapi's own fake-ACME tests).

- **Chart backend selection is new, restapi's own code default is unchanged (dev/self-hosted-with-no-chart stays `manual`).** New `mail.signingEnrollment.{backend, directoryUrl, contactEmail}` in
  `helm/values.yaml`. `backend: auto` (the default) resolves, at render time, to `rfc8823` for a real `global.domain` and `manual` for `localhost` or a domain ending in `.local`/`.localhost`/`.test`/
  `.invalid` (`rapidmx.isLocalDomain`/`rapidmx.signingEnrollmentBackend` in `_helpers.tpl`) - a public CA can't issue for those. `rfc8823`/`manual` explicit values pass through; anything else, or a
  `directoryUrl` that isn't `https://` once resolved to `rfc8823`, fails the render with a one-line explanation (`rapidmx.assertSigningEnrollment`, called from `service.yaml` alongside the existing
  `assertSuppliedSecrets`/`assertPostfixBridge` asserts). `contactEmail` empty defaults to `global.certmanager.email`. Maps to the ConfigMap as `mail__pki__signing_enrollment__backend`,
  `mail__pki__rfc8823__directory_url`, `mail__pki__rfc8823__contact_email` (`values.yaml`'s `service.config`, same pattern as every other `mail__*` key there). `helm lint` and `helm template` pass for:
  a real domain (-> `rfc8823`), `localhost` (-> `manual`), an explicit `manual` override on a real domain, a bad `backend` value (fails), and `rfc8823` with a non-`https://` `directoryUrl` (fails) -
  all four exercised by hand (`helm template` with `--set`), not as an automated test file (this chart has no helm-test harness beyond `helm lint`/`helm template` run manually). **Not independently
  verified**: `NOTES.txt`'s new "Signing certificates" section (which mode is active, that `rfc8823` needs inbound mail + DKIM) - `helm template` does not render `NOTES.txt` (only `helm install`,
  which needs cluster access this environment doesn't have); read it over by eye against the same `rapidmx.signingEnrollmentBackend`/`rrst.render` helpers the ConfigMap uses successfully.
- **Persistence confirmed**: `mail:pki:rfc8823:store_dir` (`/var/lib/rapidmx/pki/rfc8823`) is under `/var/lib/rapidmx/pki`, the existing `pki-data` PVC's mount path (`mail-storage.yaml`/`service.yaml`) -
  the ACME account key and every enrollment record survive a pod restart with no chart change needed.
- **New routes, restapi's own classes, mounted the same way every other `system`/`admin` route is** (a one-line subclass per backend): `src/{mongo,sql}/routes/SigningEnrollmentInfoRoute.ts` ->
  `GET /api/system/signing-enrollment`, `SigningEnrollmentAdminRoute.ts` -> `/api/admin/signing-enrollments` (both from `@rapidmx/restapi/{mongo,sql}`, no server-side logic). Added to
  `test/routes/MailAccessRoutes.test.ts`'s `RESTAPI_ROUTES` set (restapi already classifies them in its own `mailAccessRouteTable.ts`, "admin" kind: trusted + elevated, audited, no mail content).
- **`mail:jobs:acme_enrollment_driver:failure_audit_after`** (default 3, documented in `config.{mongo,sql}.ts`): how many checks of the certificate authority in a row must fail before restapi's
  `AcmeEnrollmentDriverJob` writes one `SIGNING_ENROLLMENT_CA_UNREACHABLE` audit entry, rather than only its own debug/warn logging.
- Runbook for switching an existing deployment from manual to automatic is in the README's new "Signing certificates" section (helm upgrade, restart, cancel the stale request in Settings > Encryption,
  request again, what to check if it stalls - the Signing Certificates admin page, `health.lastError`, server logs, inbound mail + DKIM signing).
- Files: `helm/values.yaml` (`mail.signingEnrollment.*`, three new `service.config` keys), `helm/templates/_helpers.tpl` (`rapidmx.isLocalDomain`, `rapidmx.signingEnrollmentBackend`,
...
### 2026-09-24 (Basic sign-in for Outlook and ActiveSync with app passwords)

JP: "auth-server already supports an OAuth authorization server path" - the question was what MAPI needs in the chart. Findings that led to Basic + app passwords instead: the mail server verifies with ONE key (`auth:secret`, HS256 session JWTs, and service-core refuses HS and RS algorithms together); auth-server's OAuth access tokens are RS256 (rotating signing key, JWKS, `aud` = client id), and its `oauth_bearer` strategy is internal to it. And Outlook desktop's OAuth sign-in is built around Microsoft's own authorities, so an OIDC provider of ours likely isn't usable. auth-server's `GET /api/auth/password` (`BaseAuthBasicRoute`, "the route a downstream service ... e.g. a mail server validating a legacy mail client's credentials would call") checks Basic against password/app-password secrets (an app password skips `requireMFA`) and returns `{ token, refresh, user }` (plus cookies and a session and SIGNED_IN audit entry per call).
- **`src/lib/BasicAuthJWTStrategy.ts`** wraps the real `jwt` strategy under the same name (so every `@Auth(["jwt"])` route, the plugins' included, is unchanged; no plugin release needed) and, only for `mail:basic_auth:paths` (`/mapi`, `/Microsoft-Server-ActiveSync`), tries a `Basic` header after the JWT finds nothing: hash-of-header cache of the access token (`cache_ttl_ms`, capped by the token's `exp`; a hit re-verifies the token through the real strategy), failure throttle per client address (`clientAddress()` from TieredRateLimiter - CIDR `trusted_proxies`, XFF right to left; service-core's own `NetUtils.getIPAddress` would give the gateway for everyone) and per username (429), `X-Forwarded-For` passed to auth-server (it only honors it from its own trusted proxies), 503 when auth-server or the mailbox lookup can't be reached (never counted as a wrong password). A username containing `@` is resolved by `primarySmtpAddress` (`ModelUtils.literal`, both backends) to `ownerUserUid` and sent to auth-server as `uid:password` - auth-server's `lookup()` takes a uid or an alias, not a mailbox address; a shared (ownerless) or unknown address goes as given. Aliases addresses aren't resolved.
- **Wiring:** `enableBasicAuthIfApplicable()` (`src/lib/enableBasicAuth.ts`) in `worker.ts`/`worker.mongo.ts`/`worker.sql.ts`, right after the dev auto-login and before `new Server`, like `enableDevAutoLoginIfApplicable` (it skips `yarn dev`, which replaces `jwt` outright); it `register("jwt", ...)`s over what `AuthMiddleware` built from `auth:strategy`. Off when `mail:basic_auth:enabled` is false or `mail:auth_server_url` is empty.
- **Chart:** `mail.basicAuth.{enabled,realm,cacheTtlMs,failureLimit}` -> `mail__basic_auth__*` in the server's config; `auth__app_password__enabled: true` set explicitly on the auth-server subchart (it is the default; the dependency is now visible). Checked with `helm template` (needs `--set global.mailIngestSecret=x global.jwt.secret=y global.secrets.cookies=c global.secrets.sessions=s global.openbao.enabled=false`). Nothing to add for the gateway (`Authorization` passes through) or `trusted_proxies` (already RFC1918 by default).
- **Verified:** `test/lib/BasicAuthJWTStrategy.test.ts` (26; real JWT strategy, fake auth-server `fetch`), `test/BasicAuth.mongo.test.ts` (a real `Server` with a fake auth-server: challenge header on the 401, Basic signs in, JWT unchanged, Basic ignored elsewhere - `/api/mail/mailboxes/domains` stands in for `/mapi`), and a throwaway probe with the real published activesync/mapi plugins: `/mapi/emsmdb`, `/mapi/nspi` and ActiveSync answer 401 + `WWW-Authenticate: Basic realm="RapidMX", charset="UTF-8"` with no or wrong credentials, and a valid app password gets past authentication (the 500/501 that follow are the handlers rejecting an empty request body). **Not tried with a real Outlook.** Things Outlook may still trip on after this, unverified: it wants Basic over HTTPS (the gateway terminates it), and what the MAPI handlers do with a real Connect request.
- **Not done / decisions:** JWKS-verified OAuth for native Outlook (needs a resource-server strategy in the server, MAPI/EAS `@Auth`, a `WWW-Authenticate: Bearer` challenge, an OAuth client registered in auth-server - and Outlook accepting a non-Microsoft authority); NTLM/Negotiate; a per-request auth-server call without the cache.

### 2026-09-24 (bundled coturn for the Video Conferencing plugin)

JP: bundle coturn in the server chart, secured with credentials fed into the server's config so the meet plugin is wired up automatically; then (same day) add TURN over TLS and per-join credentials. Verified: `helm lint`; `helm template` for secret / credential mode, TLS on (own certificate, `existingSecret`, other hostname), existingSecret, off, nodeSelector and each validation failure; the certificate-reloader script run on real Linux (WSL) against a stand-in `turnserver` process; `test/plugins/PluginHost.test.ts`. **Not verified: a real coturn pod** - no cluster or Docker daemon was available, so an actual relayed call, a TLS handshake and the entrypoint's expansion of `$(detect-external-ip)` are unrun. Do that on the test host (`mail.powerlevel.gg`) first. Every turnserver flag the chart passes was checked against coturn's master option table (`mainrelay.c`) and `turnserver.conf`, which caught two mistakes in the first version - see Hardening.
- **Chart** (`helm/values.yaml` `coturn`, `templates/0_config/coturn-secret.yaml`, `templates/1_deployments/coturn.yaml`, `rapidmx.coturn*` helpers): a one-replica Deployment on `hostNetwork` (`Recreate`, `ClusterFirstWithHostNet`, no Service - browsers reach it on the node's address, nothing in the cluster calls it).
- **Auth modes** (`coturn.auth.mode`): `secret` (default) is coturn's TURN REST mechanism - `--static-auth-secret=$(TURN_SECRET)` alone (source: it also sets `use_lt_credentials`; the shipped `turnserver.conf` says to use *either* `lt-cred-mech` *or* `use-auth-secret`, so `--lt-cred-mech` is not passed with it) and the plugin's existing `mail:videoconf:turn:shared_secret`, which mints `<expiry>:<random>` + base64(HMAC-SHA1) per join, valid an hour. `credential` is the static user/password JP first asked for (`--lt-cred-mech --user`). Why secret is the default: a static password is handed to every guest in the join response and stays valid; a per-join credential can't be reused after the call or used to derive another.
- **Wiring:** the Secret's keys are named for the config they become (`mail__videoconf__turn__shared_secret`, or `__username`/`__credential`) and loaded into the server pod with `envFrom`; coturn reads the same keys via `secretKeyRef` and `$(TURN_SECRET)`/`$(TURN_USERNAME):$(TURN_PASSWORD)` in its args (Kubernetes expands those), so both ends can't drift. `service.config.mail__videoconf__turn__url` is `turn:<coturn.hostname or host>:<port>`, plus `,turns:<host>:<tls.port>` with TLS. Generated values: `rrst.persistedSecret` (generated once, read back from the live Secret), `rrst.assertStableSecrets` with a new `always` key so a render without cluster access asks for it even when `global.secrets.existingSecret` is set (that Secret doesn't cover this one). Not in OpenBao: the secret is only as private as the server and coturn, and vaulting it needs the install scripts to write a new property.
- **TLS** (`coturn.tls.*`, off by default): `--tls-listening-port --cert --pkey` (else `--no-tls`, because coturn starts a TLS listener unless told not to). Certificate: `coturn.tls.existingSecret`, else the chart's own `<host>-tls-cert` (tls-certs.yaml, ECDSA P-384), which only covers `host` - so `rapidmx.assertCoturn` fails the render for another `coturn.hostname`, or when the chart issues none, unless `existingSecret` is set. HTTP-01 can't be solved for a name the Gateway doesn't route, which is why the chart doesn't try to issue a second certificate. Renewal: coturn reloads certs on SIGUSR2 (`mainrelay.c`), and cert-manager renews every 60 days, so a `certificate-reloader` sidecar (same image, `shareProcessNamespace: true`, `cksum` of the cert and key every 5 minutes, `kill -USR2` on any process whose `comm` is `turnserver`) reloads it - script tested on Linux. The pod stays `ContainerCreating` until cert-manager has created the Secret. **Opt-in because of the plugin:** the two URLs travel as one comma-separated `mail:videoconf:turn:url`, parsed by `meet-plugin` `parseTurnUrls()` (added the same day, plugin >= 0.4.0); an older plugin passes the whole string as one `RTCIceServer.urls` entry, which the browser rejects, and `new RTCPeerConnection()` throws - no call would connect.
- **The finding that made a server change necessary:** `PluginStateStore` seeds every manifest default, including `""`, into the plugin row, and `PluginHost.prepare()` then applies saved settings with `config.set()`, which outranks the environment - so at install the meet plugin's empty `mail:videoconf:turn:*` would have overridden the chart's env config. `PluginHost` now skips an empty saved value (`validatePluginSettings` already treats `""` as "remove", so an empty value never meant anything else). Consequence: the admin console can no longer force a setting to empty over env config. The chart therefore needs a server image that has this change.
- **Hardening:** all capabilities dropped except `NET_BIND_SERVICE` - the image's `turnserver` has that as a *file* capability (`setcap ...+ep`, read from the image history), and a file capability missing from the bounding set makes `execve` fail EPERM. `--denied-peer-ip` for RFC 1918, CGNAT, loopback, link-local (169.254.169.254 metadata), ULA/link-local IPv6 (coturn splits a range on the first `-` and parses each half as an address, so the IPv6 ranges are valid); `--no-tcp-relay`, `--no-multicast-peers`, `--fingerprint`. **Two flags the first version passed were wrong and would have stopped coturn starting or been noise:** `--no-dtls` is not in coturn's option table at all (DTLS is off unless `--dtls`), and `--no-cli` is deprecated (the CLI is off unless `--cli`) - both dropped. TLS version options are opt-in in current coturn, so none is passed. User name, secret, password, host and external IP are regex-validated in `rapidmx.assertCoturn` because the image's entrypoint passes the args through a shell (its default CMD is `--external-ip=$(detect-external-ip)`, so it evals them). Tag `4.18.0` (Docker Hub's latest numbered tag on 2026-09-24); I read coturn's *master* source, which may differ from that tag in small ways.
- **Open question to check on the live host:** whether coturn applies `denied-peer-ip` before or after mapping the external address to the node's private one (`--external-ip` with one address, node behind a 1:1 NAT as on AWS). If two participants who both need the relay can't reach each other, that is where to look; `coturn.deniedPeerIps` is the knob.
- **Ports:** `single_node_install.sh` opens 3478/tcp, 3478/udp, 5349/tcp and 49152-49252/udp (`firewallOpenPort` turns a range into ufw's colon form; the recorded rule is the converted one so `--uninstall` deletes it). 5349 is opened even though TLS is off by default, so turning it on later needs no firewall change. **AWS:** `bootstrap.sh` passes `coturn.create=false` and the README says why - the instance's public IP is ephemeral and the Classic ELB carries no UDP; an Elastic IP resource, security-group rules and `--set coturn.hostname/externalIp` would enable it (not done: can't test CloudFormation here).
- **Not done:** a second URL for `?transport=tcp` on 3478 (TLS covers the blocked-UDP case); TURN on 443 (the Gateway owns it on the same node); per-user quotas (in credential mode every guest shares one username, so a per-user quota would throttle everyone together).

### 2026-09-25 (clean install of beta.15 on mail.powerlevel.gg: plugin URLs, TURN, auth-server `app_url`)

JP's clean install of 1.0.0-beta.15 showed Autodiscover's `Public server URL` empty, Booking's `Public booking page URL` empty, no TURN settings on the Meet plugin, and no Return to App button on the auth-server's account page. Checked each against beta.16 with `helm template` (`global.domain=powerlevel.gg`; it needs `--set global.mailIngestSecret=x global.jwt.secret=y global.secrets.cookies=c global.secrets.sessions=s global.openbao.enabled=false coturn.auth.sharedSecret=<any>`, the last because a render without cluster access refuses to invent the coturn secret) and the published plugin manifests (`https://registry.npmjs.org/@rapidmx/<pkg>/<version>`, field `rapidmx.plugin.settings`).
- **Autodiscover (1): NOT fixed by beta.16 on its own.** The server adds `@rapidmx/autodiscover-plugin@latest` from npm, and `latest` is **1.1.0, whose `mail:autodiscover:public_url` default is `""`**; the `https://<host>` default (commit bfba613) is in the local repo's 1.1.1 (tagged v1.1.1), which is **not published to npm**. So the `<host>` seeding in `PluginStateStore.describeDefault()` has nothing to resolve. Publishing 1.1.1 is a release step not done here.
- **Booking (2): NOT fixed by beta.16.** `@rapidmx/booking-plugin` (0.5.1, latest) has never declared a `<host>` default (`""`), and the chart set no `mail__booking__public_url` - the comment in `config.*.ts` saying the chart sets it from `host` was untrue.
- **Meet (3): fixed by beta.16, and now also for rows saved before.** Published meet-plugin 0.4.1 (latest) defaults `mail:videoconf:public_url` to `https://<host>/meet` (seeded from `mail:dns:mx_hostname` = `mail.powerlevel.gg`, which the chart sets) and declares `mail:videoconf:turn:{url,username,credential,shared_secret}`, exactly the env names the chart sets (`mail__videoconf__turn__url` in the ConfigMap - `turn:mail.powerlevel.gg:3478`; `mail__videoconf__turn__shared_secret` in the `<release>-coturn` Secret, loaded into the server pod with `envFrom`). `coturn.create` is true by default and `single_node_install.sh` opens 3478/tcp+udp, 5349/tcp and 49152-49252/udp (only when a host firewall is active - a provider's security group is not touched); coturn on `hostNetwork` binds those on the node, and nothing else on the installer's host (nginx stream: 25/80/443; Envoy behind it) uses them. `PluginHost.prepare()` skips an empty saved setting, so the plugin rows the plugin seeds with `""` don't hide the env values (the fix is in the server image, so it needs beta.16's image). Meet's manifest has no `<host>` for the TURN settings, so the meet plugin's *form* still shows the TURN fields empty although the values are in effect; that is not a bug but looks like one.
- **Change (1, 2, and meet's public URL): the chart sets the three public URLs in the server's ConfigMap** (`mail__autodiscover__public_url`, `mail__booking__public_url` = `.../book`, `mail__videoconf__public_url` = `.../meet`; helper `rapidmx.serverURL` = `https://<host>`, `http://` with `global.gateway.tls` off, `host` not `mx_hostname`). It works with any plugin version and for plugin rows saved by earlier releases (empty saved values don't override config) - that is the only thing that fixes JP's already-installed host without a plugin release. Trade-offs: the form reads empty for these; an admin can't force one of them empty (clearing it sends `null`, and env applies). I did not copy env values into the plugin rows at seeding: a copy would go stale and, being saved, outrank the env value the chart changes later. `config.mongo.ts`/`config.sql.ts` comments corrected (both, CRLF preserved; they also said an env var beats a saved value - it is the other way round for a non-empty one).
- **`app_url` (4):** `authserver.service.config.app_url` in `values.yaml` = `https://<host>` (`http://` when `global.gateway.tls` is false). The subchart `tpl`s its `service.config` with *its own* `.Values`, where `host` is `authserver.host` and the parent's top-level `host` doesn't exist - only `global.*` is shared. So the value is built from a new **`global.serverHost`** (empty: `mail.<global.domain>`) and `single_node_install.sh` / `deploy/aws/bootstrap.sh` pass `--set global.serverHost="$SERVER_HOST"` next to `--set host=`. `rapidmx.assertServerHost` (called from `service.yaml`) fails the render if `host` (rendered) differs from `global.serverHost | default mail.<domain>` while `authserver.create` and `app_url` is still the values.yaml default (detected by it containing `global.serverHost`); setting `authserver.service.config.app_url` explicitly disables the check. The key stays lowercase `app_url` (the subchart emits `service.config` keys as written; auth-server reads `@Config("app_url")`; the subchart's Deployment has a `checksum/config` annotation, so its pod rolls). Consequence for people who set only `host`: the render now fails with the message naming `global.serverHost` - deliberate, the alternative was a silently wrong link.
- **Verified with `helm template`/`helm lint`:** default -> auth-server ConfigMap `app_url: "https://mail.powerlevel.gg"`, server ConfigMap `mail__autodiscover__public_url: "https://mail.powerlevel.gg"`, `mail__booking__public_url: ".../book"`, `mail__videoconf__public_url: ".../meet"`, `mail__dns__mx_hostname: "mail.powerlevel.gg"`, `mail__videoconf__turn__url: "turn:mail.powerlevel.gg:3478"`; `global.gateway.tls=false` -> all `http://`; `host=mail2.powerlevel.gg global.serverHost=mail2.powerlevel.gg` -> `https://mail2.powerlevel.gg` everywhere (turn url too); `host` alone -> the failure; `host` alone plus an explicit `authserver.service.config.app_url` -> accepted; `authserver.create=false` with another `host` -> no failure; `tlsTermination=pod` and the installer's shared-Gateway settings (`global.gateway.name/namespace`, `authserver.host`) unchanged. `helm lint ./helm` clean. **No chart unit-test convention exists** (`templates/tests` holds only the stock `test-connection` pod; no test in `test/` renders the chart), so none added. **Not verified:** anything on the live host - that the auth-server pod picks the value up and shows the button, that the plugins pick the env values up after the upgrade (needs the beta.16+ image), a real coturn pod (still the standing open item above).
- **To do outside this repo:** publish `@rapidmx/autodiscover-plugin` 1.1.1 (its `<host>` default; with it, a *fresh* install also saves the value in the row), and consider a `<host>/book` default in `@rapidmx/booking-plugin`'s manifest so its form shows the URL. Neither is needed for the values to work now.

### 2026-09-25 (booking event details, host notification, faster cancellation mail)

- Booking-plugin (unreleased, next after 0.5.1): the booking's calendar event now has `location`, description, a 15 minute reminder (the booker's invitation carries it too), and a notification is filed straight into the host's Inbox on a new booking - see that repo's NOTES 2026-09-25.
- `mail.jobs.meeting_scheduling.schedule` is `*/10 * * * * *` in `config.mongo.ts` and `config.sql.ts` (was restapi's 5 minute default), because deleting a meeting only marks it and MeetingSchedulingJob mails the CANCEL: guests waited up to 5 minutes. restapi's own default was changed to the same (unreleased, so this config line is what applies until the next restapi is published; then it is redundant but harmless). Not exercised against a running deployment.
...
### 2026-09-25 (rspamd learning: controller, password, Bayes statistics in Redis; dev bypass forwards `learn()`)

Consumes restapi a146e1c (`POST /messages/:id/report` teaches rspamd through its controller; see restapi NOTES). Nothing committed or released. Verified: `helm template` (default, `tlsTermination=pod`, `gateway.tls=false`, `existingSecret`, redis auth on, external Redis, `extraConfig`, learn off, validation failures), `helm lint ./helm` (clean; an INFO about the required password, like coturn's), the init container's script executed with a stub `rspamadm` under Git Bash's bash and WSL's dash, `yarn tsc --noEmit`, `yarn lint`, `vitest test/dev` (77/77). **Not verified: any run of rspamd** - no Docker daemon, and running the unpacked image in WSL was refused by the permission classifier, so I only read files out of the image layers (registry API) and rspamd's source on GitHub.
- **What the image is** (`rspamd/rspamd:4.1.5`, read from its config and layers): runs as uid/gid 11333 (`USER 11333:11333`), `CMD /usr/bin/rspamd -f`, env `RSPAMD_LOCAL_ADDR=*` (so `worker "controller"` binds `*:11334` and `normal` `*:11333`: `bind_socket = "{= env.LOCAL_ADDR|default('localhost') =}:..."`), EXPOSE 11332-11334, has `sh` (dash), bash, sed, cat, printf, `rspamadm`. **Default configuration lives in `/usr/share/rspamd/config`; `/etc/rspamd` holds only empty root-owned `local.d/` and `override.d/`**, which `rspamd.conf` includes over the defaults (`.include(try=true; priority=1,duplicate=merge) "$LOCAL_CONFDIR/local.d/worker-controller.inc"`; `statistic.conf` includes `local.d/classifier-bayes.conf` inside `classifier "bayes" {}`, which already has `backend = "redis"`, `min_learns = 200`, no servers). Default controller `password = "q1"` (the controller code denies `q1`/`q2` for remote access), `secure_ip` loopback only. Before this change the controller was already listening on the pod IP on 11334; it just had no Service port and the well-known password.
- **Chart:** `mail.rspamd.{learn.enabled, controller.{password,existingSecret}, bayes.{minLearns, redis.{servers,db,existingSecret,existingSecretKey}, extraConfig}}` in values.yaml. `templates/0_config/rspamd-secret.yaml` = coturn's pattern (`<release>-rspamd`, key `mail__scan__spam__rspamd__controller_password`, `rrst.persistedSecret` = generated once and read back with `lookup`, `rrst.assertStableSecrets ... always` so a render without cluster access asks for `mail.rspamd.controller.password`, nothing rendered with `existingSecret`). `service.yaml`: `envFrom` that Secret (+ its template in `checksum/secrets`). Service `rspamd` gets port 11334 (ClusterIP; the chart has no NetworkPolicies of its own, only Bitnami's, so none added). ConfigMap (values.yaml `service.config`): `mail__scan__spam__rspamd__controller_url: http://rspamd:11334`, `mail__scan__spam__learn__enabled`. Helpers `rapidmx.rspamdSecretName / rspamdRedisServers / rspamdRedisSecret / assertRspamd` (password >= 16 of `[A-Za-z0-9._~-]` with a letter: the server's nconf uses `parseValues`, so an all-digit password would become a number; Redis host:port charset; db; minLearns). The controller port is fixed at 11334 (restapi's default derivation assumes it).
- **How the password reaches rspamd:** an init container (same image, `/bin/sh -ec`) reads `CONTROLLER_PASSWORD` from the Secret, runs `rspamadm pw -p "$CONTROLLER_PASSWORD"` (pw.c: `-p/--password`, encrypts by default, prints `$<id>$<salt>$<key>`; every start gives a new salt, harmless), and writes `password = "<hash>";` to `local.d/worker-controller.inc` on an `emptyDir` that the main container mounts at `/etc/rspamd/local.d`; it writes `classifier-bayes.conf` there too (`servers`, `db`, `password` if Redis has one - `\` and `"` escaped, `min_learns`, `extraConfig` verbatim). **Fallback on purpose:** if `rspamadm pw` fails or prints something that is not `$..$..`, the password is written in plain text (rspamd's controller accepts a password that is not `$N$...` as a plain string - the `rspamd_constant_memcmp` branch of `rspamd_controller_check_password`, read in master's controller.c) and the failure is logged - because the server quarantines every message while rspamd is down, an unverified init step must not be able to take the scan worker down. Only `password` is set (not `enable_password`): with no `enable_password` the controller uses `password` for the privileged learn commands. `secure_ip` untouched (loopback). The script avoids `$(...)` because the kubelet expands that syntax in `args`.
- **Redis:** the bundled Bitnami Redis (`redis`, standalone, `auth.enabled: false`, `master.persistence.enabled: false` -> emptyDir, AOF on, `FLUSHDB`/`FLUSHALL` renamed away, no maxmemory) is used at `redis-master:6379`, derived from `global.cache.url`, **database 1** (the cache is 0). Its NetworkPolicy (`allowExternal: false`) only admits pods labelled `redis-client: "true"`: the rspamd pod now has it (when `redis.create`), like the server pod. `redis.auth.enabled` -> the Bitnami `redis` Secret's `redis-password` via secretKeyRef (`REDIS_PASSWORD`). Remember: **the statistics live on the Redis pod's emptyDir and are lost if that pod is deleted; `redis.master.persistence.enabled=true` keeps them** (left off: it is the chart's cache too, and turning it on changes the StatefulSet's storage).
- **autolearn deliberately not configured.** `lua_bayes_learn.autolearn` skips messages with no queue id (`require_queue_id = true` by default) and from local/authed networks; `RspamdSpamScanProvider.scoreMessage()` sends neither, so autolearn would never fire, and unverified thresholds could poison the statistics. So reports are the only lessons, `min_learns` stays at rspamd's 200 per class (`mail.rspamd.bayes.minLearns`), which a small deployment reaches slowly - expected, and said so in the README section "Spam filtering". `extraConfig` is the escape hatch. Global (not per-user) classifier; the `Deliver-To` header restapi sends is ignored by it.
- **Config safety check (`config.defaults.ts`):** the controller password is NOT in `assertProductionSecretsAreSet()`. Empty is a legitimate value (rspamd on loopback accepts it from `secure_ip`), and a missing or wrong password only makes learning fail, reported as `learnSkipped: "failed"` after the message has moved; it is not an authentication-forgery risk like the three guarded secrets. `config.mongo.ts`/`config.sql.ts` (CRLF kept) list `scan.spam.rspamd.{controller_url:"", controller_password:"", controller_timeout_ms:10_000}` and `scan.spam.learn.enabled: true` with comments.
- **`DevBypassSpamScanProvider.learn()`** (`src/dev/DevScanBypass.ts`): forwards to the real `RspamdSpamScanProvider.learn()`; a rejection with the same "unreachable" codes the scan bypass uses (`isEngineUnreachableError`, also as a `fetch()` cause) resolves (counted as learned) with a warning throttled by `warnIntervalMs` and a count; other errors (HTTP 4xx/5xx, an AbortError timeout) propagate -> `failed`; an inner without `learn` rejects. It does not touch `unreachableUntil` (that is the scan port's state; the controller is a different port). Types: a local `SpamLearnFn`, so it compiles against a restapi that predates `learn`. Tests: `test/dev/DevScanBypass.test.ts` "learn" (4). **The installed `node_modules/@rapidmx/restapi` (0.22.1, modified today) already has `learn`, but the published 0.22.1 does not: this needs the next restapi release and a `package.json` bump (not done; JP releases).**
- **Not touched:** `single_node_install.sh`, `deploy/aws/bootstrap.sh` (they `helm upgrade --install` against the cluster, so `lookup` works and nothing needs passing), `docker-compose.mail.yml` (its rspamd has no Redis and publishes no controller, so there a report says `failed` when something answers, or "learned" through the dev bypass when nothing listens).
- **On the live host after the upgrade:** the rspamd pod restarts (init container first; check `kubectl logs deploy/rspamd -c rspamd-config` says it wrote both files and did NOT print the plain-text warning); the classifier starts empty and acts after `minLearns` of each class; the first report exercises the chain end to end (`learned: true` in the response and `message.reported` audit details; a wrong or absent password shows as 403 in rspamd's log and `learnSkipped: "failed"`). Could still go wrong, unchecked: `rspamadm pw` in the image (the fallback covers it), rspamd honouring `local.d` on the emptyDir mount (the include lines are in the image's own `rspamd.conf`/`statistic.conf`), the `db`/`servers`/`password` keys at classifier scope (documented at docs.rspamd.com/configuration/statistic), `min_tokens = 11` rejecting a tiny message (would show as `failed`), `can_learn` guards refusing a message that already scores as its class, and the classifier's per-scan Redis lookups adding scan latency.

### 2026-09-25 - @rapidrest/react 2.0.0 and its router

JP released @rapidrest/react 2.0.0 (client-side router as a progressive enhancement of server rendering, plus breaking changes). Admin and escrow use the library router; www deliberately does not.

- **Why not www.** The library remounts the page subtree on every navigation (`RouteBoundary key={++renders}`, one `#react-root`, a server-only `_layout.tsx`), so a persistent `AppChrome` (compose windows, unlock prompt, idle-key timer, sign-out listener, user menu and rail state) would be torn down on every page change, and a folder switch (a query-only change) would remount the page. web-client keeps its bespoke router (`apps/shared/navigation/*`, `www/_routedPage.tsx`, `www/_routes.ts`) until the library gains a persistent shell.
- **Admin and escrow:** `router = true` on `AdminConsoleRoute` / `EscrowConsoleRoute` (Mongo and SQL). Every page renders its own shell, so a per-page remount costs nothing over the full page load it replaces. `ROUTER_APP_DIRS` in `src/lib/serverViteConfig.ts` lists the routed app dirs and is passed as `router: [...]` to `createViteConfig()` by both `vite.config.ts` and the PluginUiBuilder, so both builds get the `__router` entries (under `yarn dev` these consoles answer 500 until the first vite build has written them to the manifest). **Plugin routes:** `createPluginUiRoute` sets `router = false`, because a plugin subclass of `AdminConsoleRoute` would inherit `router = true` and throw for a missing router entry.
- **Breaking changes audited, nothing to fix:** no wrong-case links, no `@ReactService`, no CSS modules or imported assets in server-rendered pages, no `resolveAppFile` overrides, static export unused; only helper files are `_`-prefixed. Error redaction now follows `isDevMode()`, so under `NODE_ENV=dev` (docker-compose.mongo.yml) errors are redacted in the `_500` page (still logged). The chart and the installer's nginx do no response caching (Envoy only compresses, keeping `Vary`; nginx is a `stream {}` TCP proxy); anything in front of the server that caches must honour `Vary: X-Rapidrest-Navigation`.
- **Testing:** vitest cannot render pages (native import of TSX), so the SSR and navigation paths were checked in headless Edge against the real routes on a spare port (admin and escrow navigate on the client, `Vary` present, Ctrl-click and links out are left to the browser, www's own router still works); `test/routes/ConsoleRouter.test.ts` covers the route flags, 404s and `Vary`.
- **Traps:** `yarn install --mode=update-lockfile` rewrites only the lockfile (a plain install here would overwrite JP's unpublished local builds of the sibling packages in node_modules); the library's peerDependencies still say core 5.x and service-core 1.x (yarn peer warnings, as with beta.3); `Server.sql.test` has a 10 s hook that is timing-sensitive on a cold cache.
- **Follow-ups:** after-save redirects in the consoles (`window.location.href = "/admin/..."`, about ten) are still full page loads - converting them needs web-client to depend on `@rapidrest/react` for `useRouter().navigate`; use `Link` in the admin rail for hover prefetch; move the router features web-client had to build itself into the library (persistent shell, query-only navigation, focus/announce/pending, idle prefetch).

### 2026-09-26 - @rapidrest/react 2.1.0; www joins the router

`WwwRoute` (Mongo and SQL) sets `router = true`; `ROUTER_APP_DIRS` lists www, admin and escrow and `ROUTER_OPTIONS` (idle prefetch of `/`, `/calendar`, `/contacts`, `/tasks` with data off, plus `links: true`) is passed to `createViteConfig({ router: { appDirs, prefetch } })` by both `vite.config.ts` and PluginUiBuilder. The webmail's persistent frame is web-client's `apps/www/_shell.tsx`, which the library finds and builds into www's router entry, so the server needs the web-client release that has it (with the old web-client two routers would fight over clicks). Plugin routes still set `router = false`, so plugin pages on the www host have no shell. Options are shared by all router apps: the www idle URLs are no-ops for admin and escrow. Each www navigation costs one `WwwRoute.fetchProps()`. The 2.0.0 follow-ups (console redirects, moving router features upstream) are done. Lockfile updates use `--mode=update-lockfile`. Lint trap: `jsdoc/check-indentation` fails on an indented continuation line in a `/** */` bullet list.

### 2026-09-26 - admin Diagnostics page: server side, namespace-scoped RBAC

Nothing committed or released. UI is in web-client (`apps/admin/diagnostics`, `apps/shared/components/admin/diagnostics`); its notes are in that repo's NOTES.

- **JP's constraint: the server's Kubernetes access is namespace-scoped only - a `Role`, never a `ClusterRole`, so installers need no cluster-wide permission.** I first designed it around the kubelet (`nodes/stats` through a small ClusterRole: node CPU/RAM/disk and every PVC's usage from `/stats/summary`) and JP asked for that to go. What a Role can read: `pods`, `persistentvolumeclaims`, `metrics.k8s.io` `pods`; `GET /version` needs no grant. So: node figures are the server container's own view (`os.cpus/totalmem/freemem`, `fs.statfs` of `/`), which on a single-node k3s is the machine; PVC usage only for PVCs the server pod mounts (from its own pod spec's `volumes` and `volumeMounts`, `statfs` on the mount path); the "nodes" list is derived from the pods' `spec.nodeName` and `status.hostIP`; per-pod CPU/memory need metrics-server (404 -> reported as not installed, not an error). Do not reintroduce node or kubelet reads without asking.
- **hostPath-style PVCs (k3s local-path) report the node's filesystem**, not the volume's: `statfs` on the mount is the whole disk and the requested size isn't enforced. `sharesNodeDisk` (`metrics.ts`) flags a filesystem more than 10% away from the PVC's size; the UI notes it. Real CSI volumes are within their filesystem overhead of their size and are not flagged.
- **Layout:** `src/diagnostics/` (`KubeClient` = minimal in-cluster JSON GET over `node:https` with the service account token re-read per request, no `@kubernetes/client-node`; `kubernetesInfo` pod/node/version parsing; `components` classifies a container as postfix/postfix-bridge/mongodb/postgresql/redis/coturn/rspamd/clamav by container name first, image basename second, ignoring `exporter`/`metrics` sidecars; `metrics`, `serverInfo`, `DiagnosticsCollector`), `src/routes/BaseDiagnosticsRoute.ts` + one-line Mongo/SQL subclasses at `@ApiRoute("admin/diagnostics")`. Each part degrades to `available: false` with a reason instead of failing the request. Tests in `test/diagnostics/` and `test/routes/DiagnosticsRoute.test.ts` use an injected transport; the real `https` transport is tested against a mocked `node:https`.
- **Helm:** `templates/0_config/diagnostics-rbac.yaml` (Role + RoleBinding, rendered when `global.diagnostics.rbac.create` and the chart has a ServiceAccount). The switch is a **global** because the auth-server subchart (beta.25) has the same page and the same template and can only read globals; the first version of this (a top-level `diagnostics.rbac.create` plus helper changes that forced a ServiceAccount) left the auth-server with no SA, Role or binding, since this chart's `global.serviceAccount.create: false` overrides the subchart's own `true` (Helm: parent globals win) - found by rendering. Now `global.serviceAccount.create` defaults to true here (each chart names its own SA from its fullname; `global.serviceAccount.name` must stay empty or both would create the same name), the SA helpers are the original ones, and one pair of globals drives both charts. Verified with `helm template` (default: both charts get SA + Role + RoleBinding, no ClusterRole; `rbac.create=false`: SAs only; `serviceAccount.create=false`: none) and `helm lint`. Both pods change ServiceAccount on upgrade (from `default`) - in the release notes.
- **`/admin/logs` is `/api/admin/logs`**: `AdminRoute` is `@ApiRoute("/admin")` and `BaseAdminRoute` declares `@WebSocket("/logs")`, so it sits under `/api` (unlike the push socket at `/push`). It publishes Winston `info` objects as JSON over Redis pub/sub on channel `<service_name>-logs`, needs the `logs` datastore, and is trusted-role + elevation only; the first frame is `{id:0,type:"SUBSCRIBED"}`, permission/config failures close with code 1002.
- **Not done / not verified:** never run against a live cluster (powerlevel.gg is the k3s test host); the real API-server answers were only written against the documented shapes. Config files are CRLF in the working tree (git normalises): edit them with CRLF-aware tooling.

### 2026-09-26 - the plugin form's empty TURN fields; admin-saved settings win over the environment

Not committed. JP's screenshot: Video Conferencing settings with every TURN field empty on a chart install. Cause: the form shows the plugin row's saved settings, and the chart sets the values in the environment. JP's decisions: **saved in the admin console -> environment -> defaults, always; an empty value in the database is null, so the environment or default applies.**
- **The earlier entries (2026-09-19..24) that say `config.set()` "outranks the environment" were wrong**: nconf's layers are argv, env, memory (`set()`), defaults, so before this change an environment variable beat a saved plugin setting (verified with nconf 0.13) - an admin's edit of a key the chart sets was silently ignored. Now `config.mongo.ts`/`config.sql.ts` add a first, literal layer named `PLUGIN_SETTINGS_STORE` (`"plugins"`, `config.defaults.ts`) and `PluginHost.prepare()` writes saved settings there with `applyPluginSetting()` - not `config.set()`, which would (and still does, for every other caller) write to memory under the environment. nconf makes a literal store read-only, so the helper flips `readOnly` around the write. `PluginHost` still skips `""`, and now `null`/`undefined`, so the manifest's seeded `""` defaults don't hide the chart's values. restapi's `PLUGIN_SETTINGS_STORE` must stay the same name (it leaves that layer out of `configured`).
- The form fix is in the siblings (restapi `Plugin.configured`, react-shared types, web-client dialog + Reset) - see their NOTES; the server needs those releases (nothing bumped here). Tests: `PluginHost.test.ts` (a saved value beats env and defaults through the real layers; empty/null let them apply; `set()` stays under env), `config.defaults.test.ts` (`applyPluginSetting`). Comments in `config.*.ts` and `values.yaml` say what is true again. **Not verified: a live cluster** - that the running server's `@Config` reads pick the saved value up, and a real TURN call.
- Consequence to know: a deployment can no longer pin a plugin setting with an environment variable if an admin saves a different one; Reset in the console (or clearing the field) hands it back.
