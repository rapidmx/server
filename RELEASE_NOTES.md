...
- **The server uses `@rapidmx/restapi` 0.24.0, `@rapidmx/react-shared` 0.20.0 and `@rapidmx/web-client` 0.19.0, and the chart uses auth-server 1.0.0-beta.25.** All three packages are minor releases (so this beta is a minor step). The admin console's Diagnostics page is in web-client (from 0.18.0), and the auth-server has the same page, served through its own chart's Role (see the Diagnostics entry below).

### Added

- **The server answers the admin console's new Diagnostics page.** `GET /api/admin/diagnostics/versions`, `/runtime` and `/metrics` (trusted role and an elevated token, like the rest of `/api/admin`; `DiagnosticsRoute`, Mongo and SQL) report the server's Node.js and package versions and every installed package, the version and health of the other containers (postfix, postfix-bridge, mongodb, postgresql, redis, coturn, rspamd, clamav - their image tag and digest, restarts and node, read from their pods), the Kubernetes version and the nodes the pods run on, and live CPU, memory and disk figures. The page's live log viewer uses `/api/admin/logs`, the WebSocket `BaseAdminRoute` already had (it needs the `logs` datastore, as before). **The chart gives the server, and the bundled auth-server, read access to their own namespace only.** `global.diagnostics.rbac.create` (on by default, and read by the auth-server subchart too, whose own Diagnostics page needs the same) adds a `Role` and `RoleBinding` in each chart allowing `get` and `list` on `pods` and `persistentvolumeclaims` and on `metrics.k8s.io` `pods`, in the release namespace: no `ClusterRole`, so installing needs no cluster-wide permission. `global.serviceAccount.create` is now on by default (it was off), so the server and the auth-server run as ServiceAccounts of their own (`<release>-server` and `<release>-authserver`) instead of the namespace's `default`, which every other pod shares, and the Roles are bound to those. **On upgrade both pods restart under a new ServiceAccount**; the auth-server chart (1.0.0-beta.25) already had `create: true` in its own values, but this chart's `false` overrode it, so before this its pod ran as `default` and no Role was bound. Leave `global.serviceAccount.name` empty: both charts read it, so one name would be created twice. Set `global.diagnostics.rbac.create=false` to remove the Roles (the pages then say Kubernetes denied the request), or `global.serviceAccount.create=false` to go back to `default`. Because nodes and the kubelet's disk statistics are cluster-scoped, what the page can show is bounded by that: the node's CPU, memory and disk are the ones the server's own container sees (on a single-node k3s, the whole machine; on a larger cluster, only the node the server runs on), per-pod CPU and memory need metrics-server (k3s includes it; without it the page says so), and PVC usage is measured only for the PVCs the server pod has mounted (blob, DKIM and PKI storage). Other volumes (MongoDB, PostgreSQL) show their size but not their usage. A hostPath-style volume, such as k3s's default local-path, reports the node's disk, not its own: the page marks those, since their requested size is not enforced. Outside Kubernetes (Docker Compose, `yarn dev`), or with RBAC off, the Kubernetes parts say why they are unavailable and the rest still works. `config.mongo.ts` and `config.sql.ts` list the new `diagnostics:namespace` and `diagnostics:timeout_ms` settings. Not run against a live cluster yet.

## v1.0.0-beta.18

### Added

- **The Helm chart sets up rspamd for learning, so reporting a message as junk or not junk teaches the spam filter.** `@rapidmx/restapi`'s report route (`POST /api/mail/messages/:id/report`) teaches rspamd through its controller (`/learnspam`, `/learnham`), which needs three things the chart did not have. The rspamd Deployment and its `rspamd` Service now also publish the controller on port 11334, inside the cluster only (a ClusterIP Service; the chart has no NetworkPolicies of its own to extend). The controller has a password: `mail.rspamd.controller.password` is generated once and kept across upgrades in a new `<release>-rspamd` Secret (or read from `mail.rspamd.controller.existingSecret`, key `mail__scan__spam__rspamd__controller_password`), loaded into the server pod as `mail__scan__spam__rspamd__controller_password`, and hashed with `rspamadm pw` into rspamd's `worker-controller.inc` by a new init container when the pod starts (rspamd's default password `q1` is replaced; if `rspamadm pw` fails, the password is written in plain text, which rspamd also accepts, and the failure is logged, so the scan worker still comes up). And the Bayes classifier now has a statistics backend: the init container writes `classifier-bayes.conf` pointing at the bundled Redis (`mail.rspamd.bayes.redis.servers`, default the host of `global.cache.url`; database `mail.rspamd.bayes.redis.db`, default 1; its password when `redis.auth.enabled`), and the rspamd pod carries the label the Redis NetworkPolicy admits. The server's ConfigMap gains `mail__scan__spam__rspamd__controller_url` (`http://rspamd:11334`) and `mail__scan__spam__learn__enabled` (`mail.rspamd.learn.enabled`). `mail.rspamd.bayes.minLearns` (200, rspamd's default) is how many spam and how many not-spam messages must be learned before the classifier's verdict counts, and `mail.rspamd.bayes.extraConfig` adds any other classifier setting. **On upgrade the rspamd pod restarts** (a moment without scanning, during which the server quarantines), the statistics start empty, and the bundled Redis keeps them on the pod's own volume (`redis.master.persistence.enabled` is off), so they are lost when that pod is deleted. A render without cluster access (`helm template`, GitOps) now also needs `--set mail.rspamd.controller.password=...` (16 or more letters, digits and `. _ ~ -`, with a letter among them), as it does for coturn's secret. `config.mongo.ts` and `config.sql.ts` list the new `mail:scan:spam:rspamd:controller_url`, `controller_password`, `controller_timeout_ms` and `mail:scan:spam:learn:enabled` settings with their defaults. Needs a `@rapidmx/restapi` that has the report route. Not run against a live rspamd yet.

### Changed

- **The server uses `@rapidmx/restapi` 0.23.0, `@rapidmx/react-shared` 0.19.0 and `@rapidmx/web-client` 0.17.2.** All three are minor releases (so this beta is a minor step): the report route and blocked and safe sender lists in restapi, the client calls for them in react-shared, and in web-client the junk and sender-list UI, permanent delete, and the webmail on the client-side router and persistent shell of `@rapidrest/react` 2.1.0. web-client and the server must be upgraded together, because the server now builds the webmail as a router app.
- **The chart uses auth-server 1.0.0-beta.24** (it was 1.0.0-beta.23).
- **The admin and escrow consoles move between their pages without loading a document.** The server uses `@rapidrest/react` 2.1.0 (it was 2.0.0-beta.3) and turns on its client-side router for those two apps and the webmail (`router = true` on `AdminConsoleRoute`, `EscrowConsoleRoute` and `WwwRoute`, `ROUTER_APP_DIRS` in the client build). A click on an ordinary link between a console's pages fetches the page's data as JSON (`X-Rapidrest-Navigation`, answered with `Vary: X-Rapidrest-Navigation`) and swaps the page in; anything else, including a link to the webmail or to a plugin page, is a page load. The webmail is a router app too (`router = true` on `WwwRoute`), with web-client's `apps/www/_shell.tsx` as its persistent frame, so its compose windows, unlock prompt and other app chrome stay mounted between pages; plugin apps beneath a console or the webmail (`router = false`) hydrate page by page as before. `@rapidrest/react` 2.0.0 also makes a URL with the wrong case a 404, never serves a `_`-prefixed file as a page, hides error messages unless `NODE_ENV` is `development` or `test`, and keys the page cache by app, so the first requests after a deploy are answered from a cold cache. Anything in front of the server that caches responses must honour `Vary`.

- **Under `yarn dev`, reporting a message no longer says the spam filter was not taught when rspamd is not running.** `DevBypassSpamScanProvider` now has `learn()`: it forwards to the real provider, and a lesson whose controller can't be connected to at all (the same connection errors a scan's outage is recognised by) is dropped with a throttled `[dev] rspamd's controller is unreachable` warning and counted as learned, like a scan that is treated as clean; any other failure (a refusing controller, a timeout) still propagates and the report says `learnSkipped: "failed"`. Only with `NODE_ENV` dev, development or test; every other `NODE_ENV` uses the real provider directly.
...
- **The chart now sets the public URLs of the Autodiscover, Booking pages and Video Conferencing plugins, so they work as installed whatever version of the plugin was added.** `mail__autodiscover__public_url` (`https://<host>`), `mail__booking__public_url` (`https://<host>/book`) and `mail__videoconf__public_url` (`https://<host>/meet`) are in the server's ConfigMap, built from `host` like the auth-server's `app_url`. The `<host>` seeding added earlier only reaches a plugin whose manifest declares `https://<host>` as its default and that is first added after the server has started with it: Booking pages has never declared one (its setting defaults to empty, so booking emails carried no manage link), Autodiscover's published `latest` (1.1.0) predates its `<host>` default (which is in 1.1.1, not yet published), and a plugin row saved by an earlier release keeps the empty value it was saved with. An empty saved setting no longer overrides configuration, so these now take effect on a fresh install and on an upgrade alike. A value saved in the plugin's settings in the admin console still wins. The settings form shows only what is saved, so it can read empty for a URL that is in effect through the chart.
