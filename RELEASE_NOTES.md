...
- **The admin and escrow consoles move between their pages without loading a document.** The server uses `@rapidrest/react` 2.0.0 (it was 2.0.0-beta.3) and turns on its client-side router for those two apps (`router = true` on `AdminConsoleRoute` and `EscrowConsoleRoute`, `ROUTER_APP_DIRS` in the client build). A click on an ordinary link between a console's pages fetches the page's data as JSON (`X-Rapidrest-Navigation`, answered with `Vary: X-Rapidrest-Navigation`) and swaps the page in; anything else, including a link to the webmail or to a plugin page, is a page load. The webmail keeps its own router, and plugin apps beneath a console hydrate page by page as before. `@rapidrest/react` 2.0.0 also makes a URL with the wrong case a 404, never serves a `_`-prefixed file as a page, hides error messages unless `NODE_ENV` is `development` or `test`, and keys the page cache by app, so the first requests after a deploy are answered from a cold cache. Anything in front of the server that caches responses must honour `Vary`.
