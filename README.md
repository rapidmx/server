...
loopback address (`1.0.0-beta.6`) only. For anything beyond that, set `NODE_ENV=production` and override these in a
...
```bash
helm upgrade --install --create-namespace --namespace rapidmx rapidmx oci://ghcr.io/rapidmx/charts/server --version 1.0.0-beta.6   --set global.jwt.secret="$(openssl rand -hex 32)" --set global.mailIngestSecret="$(openssl rand -hex 32)"   --set global.domain=example.com
```
